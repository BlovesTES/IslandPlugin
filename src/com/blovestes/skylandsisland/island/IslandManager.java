package com.blovestes.skylandsisland.island;
...
public class IslandManager {
	...
	private static Location nextLocation = new Location(Skylands.getSkyBlock().world, x, 72, z);
	...
	public static void createIsland(Player p) {
		...
		z = nextLocation.getZ() + offset;
		if (x > Math.abs(IslandManager.x)) {

		}
		Location loc = new Location(Skylands.getSkyBlock().world, x, 72, z);
	...
	private void registerFile() {
		file = new File(Skylands.getSkyBlock().getDataFolder(), "IslandManager.yml");
	...
	private void registerIslandFile() {
		islandFile = new File(Skylands.getSkyBlock().getDataFolder(), "islands.yml");
	...
	private void loadNextLocation() {
		if (config.contains("nextLocation")) {
			...
			nextLocation = new Location(Skylands.getSkyBlock().world, x, 72, z);
		} else {
	...
	public void loadPlayer(Player p) {
		if (islandConfig.contains(p.getUniqueId().toString())) {
			...
			double x = s.getDouble("x");
			double z = s.getDouble("z");
			Island i = new Island(p, new Location(Skylands.getSkyBlock().world, x, 72, z), false);
	...
	public void tpToPlayersIsland(Player p, Player t) {
		p.teleport(getIsland(t).getSpawnLocation());
	}
